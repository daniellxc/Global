...
namespace LAB5GED.MVC.Controllers
{
    ...
    public class DocumentoController : Controller
    {
       ...
       [PermissaoFiltro("ListarDocumentos")]
        ...
        public ActionResult ExcluirDocumento(int _registroDocumento)
        {
            try
            {
                _DAO.ExluirDocumento(_DAO.GetByRegistro(_registroDocumento));
                return View("Index");
            }
            catch (Exception ex)
            {
                MetodosUtilidadeGeral.MensagemDeErro("Não foi possível excluir o documento. " + ex.Message, Response);
                return RedirectToAction("ExcluirDocumento", _registroDocumento);
            }
