...
namespace LAB5GED.DOMAIN.DAO.Business
{
    public class SubserieBO
    {
        ...
        public List<Subserie> GetSubseriesAtivasVinculaveis()
        {
            return _DAO.Find(ss => ss.Ativo == true && ss.AceitaDocumentos == true).OrderBy(ss => ss.Serie).OrderBy(ss => ss.Subserie_pai).OrderBy(ss => ss.Id_subserie).ToList<Subserie>();
        }
        ...
        {
            var source = from s in this.GetSubseriesAtivasVinculaveis()
                         select new Subserie { Registro = s.Registro, Serie = s.Serie, Rotulo_subserie = s.Id_subserie + " - " + s.Rotulo_subserie };
            return source.ToList<Subserie>();
        }
